...
function Search() {
  ...
  // const allCoords = { lat: 28.7, lng: -73.3 };
  let index = 0;
  ...
  useEffect(() => {
    if (restaurantsList && restaurantsList.length) {
      for (let i = 0; i < restaurantsList.length; i++) {
        let newObj = {
          lat: Number(restaurantsList[i].latitude),
          lng: Number(restaurantsList[i].longitude),
        };
    ...
    if (hotelsList && hotelsList.length) {
      for (let i = 0; i < hotelsList.length; i++) {
        let newObj = {
          lat: hotelsList[i].geoCode.latitude,
          lng: hotelsList[i].geoCode.longitude,
        };
        setAllCoords((allCoords) => [...allCoords, newObj]);
      }
    }
    if (attractionsList && attractionsList.length) {
      for (let i = 0; i < attractionsList.length; i++) {
        let newObj = {
          lat: Number(attractionsList[i].latitude),
          lng: Number(attractionsList[i].longitude),
        };
        setAllCoords((allCoords) => [...allCoords, newObj]);
      }
    }
  }, [restaurantsList, hotelsList, attractionsList]);
      ...
      <Grid container>
        ...
        <Grid item xs={12} sm={12} md={5} lg={6}>
          ...
          <div id="mapContainer" style={stylesMaps.mapContainer}>
            ...
            >
              {allCoords.length &&
                allCoords.map((place, i) => (
                  <div
                    className={stylesMaps.markerContainer}
                    lat={place.lat ? place.lat : location.state.lattitude}
                    lng={place.lng ? place.lng : location.state.longitude}
                    key={i}
                  >
                    <RoomSharpIcon style={{ color: "#d14000" }} />
                  </div>
                ))}
            </GoogleMapReact>
          </div>
